...
class ChannelDAO(BaseDAO[ChannelModel]):
    ...
    async def get_many(self, *args, **kwargs) -> Sequence[ChannelModel]:
        return await self.model.filter(*args, **kwargs).prefetch_related("user", "type").order_by("-id")

    async def get_first(self, *args, **kwargs) -> Optional[ChannelModel]:
        return await self.model.filter(*args, **kwargs).prefetch_related("user", "type").first()
